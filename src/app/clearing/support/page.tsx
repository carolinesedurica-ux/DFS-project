...
export default function ClearingSupportPage() {
  const [tickets, setTickets] = useState([
    { id: "tkt-301", subject: "Duties payment clarification Martins Drift", status: "open", date: "2026-06-30" }
  ]);
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    setSubject("");
    alert("Clearing assistance request logged. Our customs broker team will contact you shortly.");
  };
    ...
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-black text-slate-800 tracking-tight flex items-center space-x-3">
          ...
          <span>Customs Clearing Assistance</span>
        </h2>
        <p className="text-gray-500 mt-2 font-medium">
          Request direct support from our customs clearing brokers regarding SAD500 filings or duty assessments.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 bg-white border border-gray-100 shadow-sm rounded-2xl p-6 space-y-4">
          <h3 className="font-extrabold text-slate-800 text-lg border-b border-gray-100 pb-3">Open Clearing Case</h3>
          ...
          <form onSubmit={handleCreate} className="space-y-4 text-xs font-semibold text-gray-600">
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="e.g. SAD500 duty verification error"
                className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-slate-800 placeholder-gray-400 focus:outline-none focus:border-primary-royal transition-colors"
              />
            </div>
        ...
        <div className="lg:col-span-7 bg-white border border-gray-100 shadow-sm rounded-2xl p-6 space-y-4">
          <h3 className="font-extrabold text-slate-800 text-lg border-b border-gray-100 pb-3">Active Inquiries</h3>
          
          <div className="space-y-4">
            {tickets.map((t) => (
              <div key={t.id} className="p-4 bg-gray-50 border border-gray-100 rounded-xl flex items-center justify-between gap-4">
                <div>
                  <span className="font-bold text-slate-800 block text-sm">{t.subject}</span>
